...
def run_demo(args: CmdArguments):
    ...
    with create_manager_session(url=args.url, port=args.port, username=args.user, password=args.password, logger=logger) as session:
        api = PolicyObjectFeatureProfileAPI(session)
        profile_id = (
            ConfigurationFeatureProfile(session).get_sdwan_feature_profiles().filter(profile_name=PROFILE_NAME).single_or_default()
        ).profile_id
